    ...
    return (
        <div className='bg-black w-[340px] md:w-[720px] sm:w-[550px] lg:w-[900px] p-5 mt-9 rounded-2xl flex flex-col text-amber-100 '>
            ...
            <div className='flex md:justify-center items-center mb-8 gap-2 ml-5 md:ml-57 mt-3'>
                ...
                <button onClick={()=>{todos.length === 0 ? toast.error("You Don't Have any Todo") :setClearAll(true)}} className='md:ml-25 sm:ml-35 cursor-pointer hover:text-amber-200 transition-all hover:scale-103 '>clear-All</button>
            </div>
            ...
            {todos.length === 0 ? (
                ...
                <div>
                    ...
                    <p className={`${todo.status ? "italic line-through" : ""} break-all`}>{todo.text}</p>
                    {/* delete icon */}
                    ...
                    <div className='flex flex-wrap gap-4 justify-center items-center'>
                        ...
                        <span className='flex-row gap-2 flex flex-wrap'>
                            ...
                            <span onClick={()=>{navigator.clipboard.writeText(todo.text); 
                                toast('Text Copied!',{ icon: '👏',style: {borderRadius: '10px',background: '#333',color: '#fff',},});}}
                                
                                className='text-center cursor-pointer transition delay-100 duration-100 ease-in-out hover:text-gray-700 hover:font-bold hover:scale-120'>
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
